    ...
    lint: (editor) => {
      ...
      return textlint.executeOnFiles([filePath]).then((results) => {
        ...
        return messages.map((message) => {
          ...
          if (!!message.fix){
            linterMessage.solutions = [{
              position: generateRange(editor, message.line - 1, message.column - 1),
              replaceWith: message.fix.text
            }];
            const rightEndColumnPos = (message.column - 1) + (message.fix.range[1] - message.fix.range[0]);
            linterMessage.location.position[1][1] = rightEndColumnPos;
            linterMessage.solutions[0].position[1][1] = rightEndColumnPos;
          }
          ...
          if (showRuleId) {
            ...
            const breakPos = message.message.indexOf("\n");
            const escapedBreakPos = escapedMessage.indexOf("\n");
            const excerpt = message.message.substring(0, breakPos === -1 ? message.message.length : breakPos).trim();
            const escapedMessageSubstr = (escapedBreakPos === -1 ? escapedMessage : escapedMessage.substr(escapedBreakPos + 1)).replace("\n", "<br>");

            linterMessage.excerpt = excerpt;
            linterMessage.description = `${badge} ${escapedMessageSubstr}`;
          } else {
            linterMessage.excerpt = message.message;
          }
